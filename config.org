...
every new key event. In the example below, the candidate is auto-committed
when "." is entered, but not after a space (e.g. for orderless regexps), on
any key aside from space if only one candidate exists, and when the 1st
candidate is an exact match for what you have typed in the buffer on a wide variety of key presses.
      #+end_src
      #+name: auto-commit.el
      #+begin_src emacs-lisp :tangle no
...
This module enable ~global-corfu-mode~ by default. You may want to enable Corfu
...
#+end_quote
This configuration block come from Tecosaur's Emacs configuration.
#+begin_src emacs-lisp
;;; :config literate
...
#+end_src
Make sure tags in vertico are sorted by insertion
order, instead of arbitrarily (due to the use of
group_concat in the underlying SQL query).
#+begin_src emacs-lisp :tangle no
...
(use-package! lsp-bridge
  ...
  (dolist (hook (-remove-item nil lsp-bridge-default-mode-hooks))
    (add-hook hook (lambda ()
                     ...
                     (setq-local +lookup-definition-functions '(lsp-bridge-jump)
...
(dolist (project '([tailwindcss "package.json"] ; `lsp-bridge--react-project-p'
                   [vue "package.json"])) ; `lsp-bridge--vue-project-p'
  (let ((lang (elt project 0))
        (dominating-file (elt project 1)))
    (defalias (make-symbol (format "lsp-bridge--%s-project-p" lang))
      (lambda (project-path)
...
(package! lsp-bridge
  :recipe (:host github :repo "thaenalpha/lsp-bridge"
