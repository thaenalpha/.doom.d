...
- [[#autoload][Autoload]]
  - [[#random][Random]]
    - [[#one-or-another][One or Another]]
    - [[#random-a-character][Random a character]]
...
- [[#modules][Modules]]
  ...
  - [[#editor][Editor]]
    ...
    - [[#substitute-substituteel][#Substitute (substitute.el)]]
  - [[#emacs-1][Emacs]]
    - [[#ctrlf-1][ctrlf]]
    - [[#dired][dired]]
    - [[#info][info]]
  ...
  - [[#tools-tools][Tools :tools:]]
    ...
    - [[#make-prodigy-sudo-able][Make Prodigy sudo-able]]
...
#+end_src
** Startup
- *[[elisp:(doom/open-scratch-buffer)][doom:scrath]]*
 #+begin_quote
...
#+end_src
Keybinds to the above commands with [[kbd:][<leader> t]] prefix.
#+begin_src emacs-lisp :tangle no :noweb-ref doom-leader-toggle-map
...
** Leader [[kbd:][SPC]]
*** Open [[kbd:][o]]
Keyboard Shortcut to open apps with [[kbd:][<leader> o]] prefix.
#+begin_src emacs-lisp :tangle no :noweb-ref doom-leader-open-map
:desc "Calc"              "c"     #'calc
:desc "APP: rss"          ","     #'=rss
#+end_src
...
* Autoload
This section contains the code + autoload functions in =autoload.el= or
=autoload/*.el= to get the benefits of lazy loading.

+ What "autoload" means?
  From Emacs manual Concept:
  #+begin_quote
Some commands are “autoloaded”; when you run them, Emacs automatically loads the
associated library first. For instance, the ‘M-x compile’ command (*note
Compilation::) is autoloaded; if you call it, Emacs automatically loads the
‘compile’ library first. In contrast, the command ‘M-x recompile’ is not
autoloaded, so it is unavailable until you load the ‘compile’ library.
  #+end_quote
  This also means that the Emacs load time can be saved by lazy load of
  libraries, it will loads only when it needs. It is available because it's load
  path was indexed at ~doom sync~ time.

** Random
#+begin_src emacs-lisp :tangle autoload/random.el
...
** String Manipulation
#+begin_src emacs-lisp :tangle autoload/string.el
...
*** [[https://git.sr.ht/~protesilaos/substitute#substitute-substituteel][#]]Substitute (substitute.el)
:PROPERTIES:
:CUSTOM_ID: Substitute (substitute.el)
:END:

Efficiently replace targets in the buffer or context.

*Video demo:* [[https://protesilaos.com/codelog/2023-01-16-emacs-substitute-package-demo/][https://protesilaos.com/codelog/2023-01-16-emacs-substitute-package-demo/]]
#+begin_src emacs-lisp :tangle modules/editor/substitute/config.el :mkdirp yes
...
This part is taken from [[doom-user:][@hlissner]]
...
+ Extend [[help:org-roam-capture-templates][org-roam-capture-templates]] to include the new templates.
