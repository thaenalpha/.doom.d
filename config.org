...
<<my-frame-settings>> <<pointer-settings>>
...
** Frame
Prevents some cases of Emacs flickering when opening a new frame
#+name: frame-creation-values
#+begin_src emacs-lisp :tangle no :noweb no-export
'((inhibit-double-buffering . t) <<frame-size>>)
#+end_src

by add ~inhibit-double-buffering . t~ and [[frame-size][prefer sizes]] to the ~default-frame-alist~.
#+name: my-frame-settings
#+begin_src emacs-lisp :tangle no :noweb no-export
(dolist (params <<frame-creation-values>>)
